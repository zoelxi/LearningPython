...
class UnoCard:
    ...
    attributes:
      rank: int from 0 to 9 or string representing action
      color: string'''
    ...
    def find_rank(self):
        '''UnoCard.find_rank() -> int or str
        returns rank of Uno card'''
        return self.rank
    ...
    def __init__(self):
        ...
        for color in ['red', 'blue', 'green', 'yellow']:
            ...
            for i in range(2):
                ...
                for action in ['skip','reverse','draw two']: # two of each of action of each color
...
class UnoPile:
    ...
    attributes:
      ...
      action: int indicating whether effect of skip or draw two card has been used'''
    ...
    def get_action(self):
        '''UnoPile.get_skip() -> int
        returns int indicating whether skip or draw two effect has been used'''
        return self.action

    def inc_action(self):
        '''UnoPile.inc_skip() -> int
        indicates skip or draw two effect has not been used'''
        self.action += 1

    def dec_action(self):
        '''UnoPile.dec_skip() -> int
        indicates skip or draw two effect has been used'''
        self.action -= 1
...
class UnoPlayer:
    ...
    def play_card(self,card,pile):
        ...
        and indicates skip or draw two effect has not been used if played
        CAUTION: does not check if the play is legal!'''
        self.hand.remove(card)
        pile.add_card(card)
        # indicates skip or draw two effect has not been used
        if card.find_rank() in ['skip','draw two']: 
            pile.inc_action()
...
def play_uno(numPlayers):
    ...
    while True:
        ...
        if playerList[currentPlayerNum].has_won():
            ...
            break
        # check action cards and go to the next player
        revCards = [card for card in pile.cards() if card.find_rank() == 'reverse']
        # check whether num of reverse cards in pile is odd or even to determine play order
        if len(revCards) % 2 == 1:
            playOrd = -1
            # ensure current player num is never negative
            if currentPlayerNum == 1:
                currentPlayerNum += numPlayers
        else:
            playOrd = 1
        if pile.top_card().find_rank() in ['skip', 'draw two']:
            action = pile.get_action() 
            if action == 1: # action effect has not been used
                if pile.top_card().find_rank() == 'draw two':
                    # next player draws two cards from deck
                    for i in range(2):
                        playerList[(currentPlayerNum + playOrd) % numPlayers].draw_card(deck)
                # skip next player's turn
                currentPlayerNum = (currentPlayerNum + 2*playOrd) % numPlayers
                pile.dec_action() # action effect has been used
            else:
                currentPlayerNum = (currentPlayerNum + playOrd) % numPlayers
        else:
            currentPlayerNum = (currentPlayerNum + playOrd) % numPlayers
