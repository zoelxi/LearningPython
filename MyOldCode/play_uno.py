...
class UnoCard:
    ...
    def is_match(self,other):
        ...
        return (self.color == other.color) or (self.rank == other.rank) or self.is_card('regular','Wild')

    def is_rank(self,qRank):
        return self.rank == qRank

    def is_color(self,qColor):
        return self.color == qColor

    def new_color(self,newColor):
        self.color = newColor

    def is_card(self,qRank,qColor):
        return self.rank == qRank and self.color == qColor
...
class UnoDeck:
    ...
    def __init__(self):
        ...
        for i in range(4):
            self.deck.append(UnoCard('regular','Wild'))
...
class UnoPlayer:
    ...
    def take_turn(self,deck,pile):
        ...
        if len(matches) == 0:
            for card in self.hand:
                if card.is_card('draw four','Wild'):
                    matches.append(card)
        if len(matches) > 0: # can play
            ...
            if newcard.is_match(topcard) or newcard.is_card('draw four','Wild'): # can be played
    ...
    def computer_take_turn(self,deck,pile):
        print("It's " + str(self.name) + "'s turn.")
        topcard = pile.top_card()
        matches = [card for card in self.hand if card.is_match(topcard)]
        if len(matches) == 0:
            for card in self.hand:
...
def play_uno(numPlayers):
    ...
    for n in range(numPlayers):
        ...
        while comp not in ['y','n']:
            comp = input('Would you like ' + str(name) + ' to be a computer-controlled player? (y/n) ')
        if comp == 'y':
            compList.append(UnoPlayer(name,deck))
    ...
    while True:
        ...
        for player in playerList:
            print(player)
        print('-------')
        if playerList[currentPlayerNum] in compList:
            playerList[currentPlayerNum].computer_take_turn(deck,pile)
        else:
            playerList[currentPlayerNum].take_turn(deck,pile)
        if (pile.top_card()).is_color('Wild'):
            if playerList[currentPlayerNum] in compList:
                maxColor = 0
                for color in ['red','blue','green','yellow']:
                    numColor = 0
                    for card in playerList[currentPlayerNum].get_hand():
                        if card.is_color(color):
                            numColor += 1
                            if numColor > maxColor:
                                maxColor = numColor
                                playColor = color
            else:
                playColor = input('What color do you want to play? ')
            (pile.top_card()).new_color(playColor)
        # check for a winner
        ...
        # go to the next player
            ...
            if (pile.top_card()).is_rank('draw four'):
                for i in range(4):
